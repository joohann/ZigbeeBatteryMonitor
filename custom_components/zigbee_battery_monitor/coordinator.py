...
class ZigbeeBatteryCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            devices.sort(
                key=lambda d: (d["battery_pct"] is None, d["battery_pct"] or 999)
            )

            summary = {
                BATTERY_LEVEL_CRITICAL: [
                    d for d in devices if d["level"] == BATTERY_LEVEL_CRITICAL
                ],
                BATTERY_LEVEL_LOW: [
                    d for d in devices if d["level"] == BATTERY_LEVEL_LOW
                ],
                BATTERY_LEVEL_WARNING: [
                    d for d in devices if d["level"] == BATTERY_LEVEL_WARNING
                ],
                BATTERY_LEVEL_OK: [
                    d for d in devices if d["level"] == BATTERY_LEVEL_OK
                ],
                BATTERY_LEVEL_UNAVAILABLE: [
                    d for d in devices if d["level"] == BATTERY_LEVEL_UNAVAILABLE
                ],
                "all": devices,
                "total": len(devices),
            }
